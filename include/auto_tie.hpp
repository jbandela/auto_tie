...
struct auto_tie_helper {
	...
	template<class... T>
	auto construct(T&&... t) {
		using type = std::decay_t<decltype(*std::declval<F>()(std::forward<T>(t)...))>;
		return type{ std::forward<T>(t)... };
	}
	template<class T, std::size_t... I>
	auto construct(T&& t, std::index_sequence<I...>) {
		return construct(std::get<I>(std::forward<T>(t))... );
	}
...
#define AUTO_TIE_HELPER3(r, data, i, elem) BOOST_PP_COMMA_IF(i) elem
#define AUTO_TIE_HELPER4(r, data, i, elem) BOOST_PP_COMMA_IF(i) std::move(elem)
...
#define AUTO_TIE0(seq) auto_tie([]( BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_HELPER1, _ , seq ) ) { \
	struct f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct { BOOST_PP_SEQ_FOR_EACH(AUTO_TIE_HELPER2, _, seq) \
	 auto as_tuple_ref() const{ return std::tie(BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_HELPER3, _, seq));}	\
	 auto as_mutable_tuple_ref() { return std::tie(BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_HELPER3, _, seq));}	\
	 auto to_tuple() &&{ return std::make_tuple(BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_HELPER4, _, seq));}	\
	}; \
    return static_cast<f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct*>(nullptr);\
})

#define AUTO_TIE(...) AUTO_TIE0(BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__) )

#define AUTO_TIE_REF0(seq) auto_tie([]( BOOST_PP_SEQ_FOR_EACH_I(AUTO_TIE_REF_HELPER1, _, seq )  ) { \
	struct f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct { BOOST_PP_SEQ_FOR_EACH(AUTO_TIE_REF_HELPER2, _, seq )}; \
    return static_cast<f1f067cb_03fe_47dc_a56d_93407b318d12_auto_tie_struct*>(nullptr);\
})

#define AUTO_TIE_REF(...) AUTO_TIE_REF0( BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__) ) 
